...
\begin{tikzpicture}
\begin{axis} [title={2},xmin=-4, xmax=4, ymin=-1, ymax=1]
\addplot[color=black]{1/(1 + exp(-1* x ) };
\end{axis}
\end{tikzpicture}


\begin{tikzpicture}
\begin{axis} [title={3},xmin=-4, xmax=4, ymin=-1, ymax=2]
\addplot[mark=none] coordinates {(-4, 0) (-0.1, 0) (0.1,1) (4, 1)};
\end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}
\begin{axis} [title={4},xmin=-4, xmax=4, ymin=-1, ymax=2]
\addplot [color=black]{\gauss{0}{0.5}};
\end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}
    \fill[fill=red] (3,0) -- plot [domain=3:7] (\x,{exp(-1*\x/10 )}) -- (7,0) -- cycle;
    \draw plot[domain=0:10] (\x,{exp(-1*\x/10 )});

		\draw[very thin, gray!30, step=1 cm](0,0) grid (10,2);
    ...
    \draw[yshift=1 cm, thick] (-1pt,0pt)--(1pt,0pt) node[left] {$1/10$};
\end{tikzpicture}
...
Here is a sketch of the CDF:

\begin{tikzpicture}
    \draw plot[domain=0:10] (\x,{1 - exp(-1*\x/10 )});
    ...
    \draw [thick] [->] (0,0)--(0,2) node[above, left] {$y$};
   \draw[yshift=1 cm, thick] (-1pt,0pt)--(1pt,0pt) node[left] {$1$};
\end{tikzpicture}
\printbibliography{}
\end{document}
