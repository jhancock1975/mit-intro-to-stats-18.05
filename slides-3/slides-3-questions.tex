...
\node[end, label=] {}
    child {
        ...
        child {
          node[bag] {$N \cap C$}
          edge from parent node[above] {$P \left( N \mid C \right) = $}
        }
        child {
          node[bag] {$G_{2} \cap C_{1}$}
          edge from parent node[above]  { $P \left( S \mid C \right) =  $}
        }
        edge from parent node[above] {$P \left( C \right) = \frac{1}{3}$}
    }
    child {
        node[bag] {$G$}
        child {
          node[bag] {$N \cap G$}
          edge from parent node[above] {$P \left( N \mid G \right)$}
        }
        child {
          node[bag] {$S \cap G$}
          edge from parent node[above]  {$ P \left( S \mid G_{1}$}
        }
        edge from parent node[above]  {$P \left( G_{1} \right) = \frac{2}{3}$}
    };
\end{tikzpicture}
