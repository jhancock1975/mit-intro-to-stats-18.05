...
\cite{packageClash}, \cite{curlyBrace} to write the \LaTeX source code for this document.
...
\begin{equation}
a^{x+y} = \underbrace{aaa \ldots a}_{text{x+y a's}}.
\end{equation}

We use the associative property of multiplication to state that the
above is true if and only if:
...
Now we can apply equation \rec{covarForm} again to the equation
above:
