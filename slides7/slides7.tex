...
$g\left(y, \right) = e^{-2y}$.  We use the the identity
\begin{equation}
  a^{x+y} = a^x+a^y.
\end{equation}

This identity implies that $e^{-3x} e^{-2y}=e^{-3x-2y}$.  Therefore we can
...
pdf, then they cannot be indpendent. This is because
\begin{equation}
  \frac{1}{2}\left(x^3y + xy^3 \right) = \frac{1}{2}\left(xy right)(x^2 + y^2)
  = \frac{1}{2}\left(xy \right)\left(x +y \right)\left( x - y \right).
\end{equation}

Furthermore we cannot factor the terms of the product above into simpler expressions
in $x$ and $y$, but each term involves $x$, and $y$.
