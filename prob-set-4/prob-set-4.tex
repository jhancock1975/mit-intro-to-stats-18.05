...
We know how to calculate $P\left(X < x \riight)$ as a definite integral
\cite{reading5b}, therefore we will calculate
$P\left( X \geq x \right)$
...
\begin{equation}
P\left(X \geq x \right) = -e^{-\lambda x} - -e^{-\lambda 0}.
\end{equation}

Now we simplify the previous equation:

\begin{equation}
P\left(X \geq x \right) = -e^{-\lambda x} + 1.
\end{equation}
